...
# 需要根据所需路径进行更改
path = {
    'data_save': 'D:/TSL_Download-master/data/'
}

# 当磁盘剩余空间不足时， 停止程序
min_tolerance_space_mb = 100


def get_free_space_mb():
    if platform.system() == 'Windows':
        free_bytes = ctypes.c_ulonglong(0)
        ctypes.windll.kernel32.GetDiskFreeSpaceExW(ctypes.c_wchar_p('D:/'), None, None, ctypes.pointer(free_bytes))
...
def write_file_from_dict(data, dir, file_name):
    ...
    os.makedirs(dir, exist_ok=True)
    df.to_csv(dir+file_name, index=False, encoding="GBK")
...
def code_get_day_data(T, stock_name, set_time_cycle_string, arg_string, table, down_dir, type, isOption = False):
    if not isOption:
        folder_name = "'%s' + datetimetostr(t) + '/'" % down_dir
    else:
        folder_name = "'%s'" % down_dir
    ...
    code = '''
            ...
            if not istradeday(t) then return 1;
            BegTT := ref(sp_time(),1)+18/24;
            EndTT := t+18/24;
            ...
            if istable(data) then rdo2 write_file(data, %s, bk+'_%s'+'.csv');                        
            return 1;
        ''' % (T, stock_name, set_time_cycle_string, arg_string, table, folder_name, type)
    return code


def get_data(c, begT, endT, args_tick, args_k, down_dir, stock_name, time_cycle, T_start = '', isOption = False):
    # 抽样
    global I
    I += 1
    if(I % 10 != 0):
    ...
    for t_index in range(len(Tarr)-1, -1, -1):
        if(get_free_space_mb() < min_tolerance_space_mb):
            print(get_free_space_mb())
            return [Tarr[t_index], 'noFreeSpace']
        ...
        while(not daydata_isover and is_T_start):
            if(c.exec(code = code_get_day_data(Tarr[t_index], stock_name, set_time_cycle_string_k, arg_string_k, table_k, down_dir+'k/', 'k', isOption)).value()
                and
                c.exec(code = code_get_day_data(Tarr[t_index], stock_name, set_time_cycle_string_tick, arg_string_tick, table_tick, down_dir+'tick/', 'tick', isOption)).value()):
                daydata_isover = True
            else:
                time.sleep(10)              
                error_time += 1
...
def down_future_data(c, output_dir, begT, endT, time_cycle, future_start = '', future_name_start = '', T_start = ''):
    origin_dir = output_dir + 'future/'
    print(origin_dir)

    if os.path.exists(origin_dir + 'future_AllPZCode.csv'):
        future_AllPZCode = pd.read_csv(origin_dir + 'future_AllPZCode.csv', encoding='ISO-8859-1')
    else:
    ...
    if os.path.exists(origin_dir + 'future_L_and_DL_StockNames.csv'):
        future_L_and_DL_StockNames = pd.read_csv(origin_dir + 'future_L_and_DL_StockNames.csv', encoding='ISO-8859-1')
    else:
    ...
    if os.path.exists(origin_dir + '上市期货;退市期货_baseInfo.csv'):
        future_baseInfo = pd.read_csv(origin_dir + '上市期货;退市期货_baseInfo.csv', encoding='ISO-8859-1')
    else:
    ...
    for future in future_AllPZCode.iloc[:, 0]:
        ...
        for future_name in future_L_and_DL_StockNames.iloc[:, 0]:
            ...
            if future_name.upper()[0:len(future)] == future and future_name.upper()[len(future)].isnumeric():
                Ans = get_data(c = c,
                        begT = begT,
                        endT = endT, 
                        args_tick = tick_args,
...
def down_option_data(c, output_dir, begT, endT, time_cycle, op_start = '', op_name_start = '', T_start = ''):
    origin_dir = output_dir + 'option/'
    ...
    for term in OP_UnderlyingSecurity.iloc[:, 0]:
        ...
        code = '''
            return MarketTradeDayQk(%s, %s);
        ''' %(begT, endT)
        Tarr = c.exec(code = code).value()

        for t_index in range(len(Tarr)-1, -1, -1):
            t = Tarr[t_index]

            op_ddl_dir = op_dir + str(c.exec(code='return datetimetostr(%s);'%t).value()) + '/'
            opChain_data = pd.DataFrame(c.exec(code_OP_GetOptionChain(term, t)).value())

            if(opChain_data.empty):
                continue
            else:
...
if __name__ == "__main__":
    ...
    future_ans = down_future_data(c, path['data_save'], begT = '20230507T', endT = '20230510T', time_cycle='cy_30m()')
    while(future_ans != True):
        if len(future_ans[2]) != 1:
            print(future_ans[2][1])
            sent_email('future download error, NO FREE SPACE,  stop at ' + str(future_ans))
            break
        ...
        time.sleep(5)
        future_ans = down_future_data(c, path['data_save'], begT = '20230507T', endT = '20230510T', time_cycle='cy_30m()',
                                       future_start=future_ans[0], future_name_start=future_ans[1], T_start=future_ans[2])
    time_future_used = datetime.today()           
 

    option_ans =  down_option_data(c, path['data_save'], begT='20230507T', endT='20230510T', time_cycle='cy_30m()')
    while(option_ans != True):
        if len(option_ans[2]) != 1:
            print(option_ans[2][1])
            sent_email('option download error, NO FREE SPACE,  stop at ' + str(option_ans))
            break
        ...
        time.sleep(5) 
        option_ans =  down_option_data(c, path['data_save'], begT='20230507T', endT='20230510T', time_cycle='cy_30m()',    
                                       op_start=option_ans[0], op_name_start=option_ans[1], T_start=option_ans[2])
